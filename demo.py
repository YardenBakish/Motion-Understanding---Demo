...
def parse_args():
    ...
    parser.add_argument('--mode', type=str,
                                ...
                                default = 'finetune_joint_learning_extended_mask_emph',
                                help='')
    ...
    finedtuned_dir   = f'{args.paths[args.dir_type]}/{work_dir}' #TOCHANGE
    eval_dir         = f"outputs/{work_dir}"
    ...
    args.prompt_orig     = "Describe this video in detail"
...
def concat_frames(base_dir):
    ...
    for subdir in os.listdir(base_dir):
        ...
        for folder_type in ['displacements', 'norm_maps']:
            finetuned_path = os.path.join(subdir_path, 'finetuned', folder_type)
            orig_path = os.path.join(subdir_path, 'orig', folder_type)
            save_path = os.path.join(subdir_path, folder_type)

            os.makedirs(save_path, exist_ok=True)

            # Get sorted list of frame files
            frames = sorted(os.listdir(finetuned_path))

            for frame_file in frames:
                finetuned_frame = os.path.join(finetuned_path, frame_file)
                orig_frame = os.path.join(orig_path, frame_file)

                if not os.path.isfile(finetuned_frame) or not os.path.isfile(orig_frame):
                    continue

                # Open images
                img1 = Image.open(finetuned_frame)
                img2 = Image.open(orig_frame)

                # Concatenate horizontally
                concatenated = Image.new('RGB', (img1.width + img2.width, img1.height))
                concatenated.paste(img1, (0, 0))
                concatenated.paste(img2, (img1.width, 0))

                # Save concatenated image
                concatenated.save(os.path.join(save_path, frame_file))
...
def vis(args):
    ...
    for model_type in model_types:
        ...
        for i, sample in enumerate(arr_to_test):
            ...
            paths         = [f"samples/{sample}/{videos[i]}" for i in range(len(videos))]
            ...
            for i in range(len(explainations)):
                ...
                inputs["video_id"] = f"{output_dir}/{model_type}"
                os.makedirs(inputs["video_id"],exist_ok=True)
                os.makedirs(f"{inputs['video_id']}/displacements",exist_ok=True)
                os.makedirs(f"{inputs['video_id']}/norm_maps",exist_ok=True)
    ...
    if "BB" not in args.mode:
        for i, sample in enumerate(arr_to_test):
            for j in range(len(explainations)):
                ...
                explain_baseline_original =  f"Baseline: {explainations_compare_dict['orig'][sample][j]}" 
                ...
                if False:
                    ...
                    create_video_with_text([paths_global[i],paths_global[i]], explainations_models, f"outputs/joint_learning_extended_mask_emph/2.2B/{sample}/compare_models{ext}.mp4")
    
        
        concat_frames("outputs/joint_learning_extended_mask_emph/2.2B/")
